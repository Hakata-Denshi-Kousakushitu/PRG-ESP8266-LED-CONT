...
char Version[] = "Ver2.0";
char module_name[8] = "No.xxxx";

#define PIN 5 //GPIO5
#define LED 4 //GPIO4
#define DIPSW_0  12 //GPIO12
#define DIPSW_1  13 //GPIO13
#define DIPSW_2  14 //GPIO14
#define DIPSW_3  16 //GPIO16
...
void setup() {
  ...
  pinMode(LED, OUTPUT);
  ind_ip = device_id();
  IPAddress ip(192, 168, 2, ind_ip + 81); //192.168.2.81～192.168.2.89
  sprintf(module_name, "No.%u", ind_ip);
  /*
  while (!Serial) {
...
uint8_t device_id(void)
{
  ...
  pinMode(DIPSW_0, INPUT_PULLUP);
  pinMode(DIPSW_1, INPUT_PULLUP);
  pinMode(DIPSW_2, INPUT_PULLUP);
  pinMode(DIPSW_3, INPUT);

  delay(10);
  return ((digitalRead(DIPSW_3) << 3) | (digitalRead(DIPSW_2) << 2) | (digitalRead(DIPSW_1) << 1) | digitalRead(DIPSW_0));
}
